""" Adding an IM to further diesntangle the texture and the colour
- given that CNN are texture biased it is important to disentangle texture from other FoVs
"""

import torch
from torch import nn

from cgn_framework.utils import get_norm_layer, init_net, choose_rand_patches, Patch2Image, RandomCrop
...
class TxC_CGN(nn.Module):
    ...
    def __init__(self, n_classes=10, latent_sz=32, ngf=32,
        ...
        self.f_text2 = nn.Sequential(*texture_layers(inp_dim, 1, ngf, init_sz), nn.Tanh())
        self.f_text_col = nn.Sequential(*texture_layers(inp_dim, 3, ngf, init_sz), nn.Tanh())
...
"""
The remainder of the program generates counterfactual images for the MNIST dataset and plots them.
 - also visualises the separate texture IMs
"""
