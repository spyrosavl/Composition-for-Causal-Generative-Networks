...
class DoubleColoredMNIST(Dataset):
    ...
    def __init__(self, train=True):
        ...
        # shift colors by X
        color2 = [color1[i-6] for i in range(len(color1))]

        def get_rgb(x):
            t = torch.tensor(ImageColor.getcolor(x, "RGB"))/255.
            return t.view(-1, 1, 1)

        self.background_colors = list(map(get_rgb, color1))
        self.object_colors = list(map(get_rgb, color2))
    ...
    def __getitem__(self, idx):
        ...
        ret = {
            ...
            'bg_labels': torch.Tensor([bg_color_label]).to(torch.long),
            'texture_labels': torch.Tensor([texture_color_label]).to(torch.long),
        }
        return ret
